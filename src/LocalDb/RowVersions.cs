    ...
    public static async Task<Dictionary<Guid, ulong>> Read(SqlConnection connection)
    {
        var sql = """
            ...
            WHERE t.TABLE_TYPE = 'BASE TABLE'
              AND EXISTS (SELECT 1 FROM INFORMATION_SCHEMA.COLUMNS c WHERE c.TABLE_NAME = t.TABLE_NAME AND c.TABLE_SCHEMA = t.TABLE_SCHEMA AND c.COLUMN_NAME = 'Id')
              AND EXISTS (SELECT 1 FROM INFORMATION_SCHEMA.COLUMNS c WHERE c.TABLE_NAME = t.TABLE_NAME AND c.TABLE_SCHEMA = t.TABLE_SCHEMA AND c.COLUMN_NAME = 'RowVersion');
